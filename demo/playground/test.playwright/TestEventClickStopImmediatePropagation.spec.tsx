...
test('TestEventClickStopImmediatePropagation component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        const element = h('div', null,
            h('h3', null, 'Event - Click - Stop Immediate Propagation'),            h('p', {}, "[observable-content]")
        )
        
        // Render to body
        render(element, document.body)
        
        // Define incrementOuter function
        const incrementOuter = () => outer(prev => {
            // Toggle logic would be implemented based on source
            return typeof prev === 'boolean' ? !prev : typeof prev === 'number' ? prev + 1 : prev + '_updated'
        })
        ;(document.body as any)['incrementOuterTestEventClickStopImmediatePropagation'] = incrementOuter
    })
    ...
    // Manually trigger the incrementOuter function 4 times (replacing useInterval)
    await page.evaluate(() => {
        (document.body as any)['incrementOuterTestEventClickStopImmediatePropagation']()
    })
