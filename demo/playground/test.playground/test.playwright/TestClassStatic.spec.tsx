...
test('Class Static component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        // [Implementation based on source file: TestClassStatic.tsx]

        // Create the component element using h() function - static content
        const element = h('div', null,
            h('h3', null, 'Class - Static'),
            h('p', { class: { red: true, blue: false } } as any, 'content')  // Use object for static class attribute
        )
    ...
    await expect(innerHTML).toBe('<h3>Class - Static</h3><p class="red">content</p>')  // Expected output from source
})