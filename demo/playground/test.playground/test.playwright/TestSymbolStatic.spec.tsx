...
test('Symbol Static component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        // Source has <p>{Symbol()}</p> which renders as <p></p> since symbols are not rendered
        // Expected output: <p></p>

        // Create the component element using h() function - static content
        const element = h('div', null,
            h('h3', null, 'Symbol - Static'),
            h('p', null, Symbol())  // Symbol renders as empty content
        )
    ...
    await expect(outerHTML).toBe('<p><!----></p>')  // Symbol renders as placeholder
})
