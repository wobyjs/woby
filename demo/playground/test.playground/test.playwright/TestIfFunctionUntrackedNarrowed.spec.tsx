...
declare global {
    interface Window {
        testIfFunctionUntrackedNarrowed: import('woby').Observable<number>
    }
}

test('If - Function Untracked Narrowed component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        // [Implementation based on source file: TestIfFunctionUntrackedNarrowed.tsx]

        const o = $(true)
        const content = $(0)
        window.testIfFunctionUntrackedNarrowed = content  // Make observable accessible globally
    ...
    await page.evaluate(() => {
        const content = window.testIfFunctionUntrackedNarrowed
        const increment = () => content(prev => (prev + 1) % 3)
        increment()
    })
    await page.waitForTimeout(50)
