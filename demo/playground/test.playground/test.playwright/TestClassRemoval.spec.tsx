...
declare global {
    interface Window {
        testObservableClassRemoval: import('woby').Observable<any>
    }
}
...
test('Class Removal component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        // [Implementation based on source file: TestClassRemoval.tsx]

        // Create observable with initial value true
        const o = $<boolean | null>(true)

        // Create the component element using h() function
        const element = h('div', null,
            h('h3', null, 'Class - Removal'),
            h('p', { class: { red: o } } as any, 'content')  // Use object for class attribute with observable that can be null
        )
    ...
    expect(innerHTML).toContain('<h3>Class - Removal</h3>')
    expect(innerHTML).toContain('<p class="red">content</p>')  // Expected when initial value is true
})