...
test('Cleanup Inner Portal component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        // [Implementation based on source file: TestCleanupInnerPortal.tsx]

        // Create the TestCleanupInner component logic
        ...
        const element = h(Portal, { mount: document.body } as any,
            h(TestCleanupInner, null)
        )
    ...
    await expect(innerHTML).toBe('<div><h3>Cleanup - Inner</h3><p>page1</p><button>Toggle Page</button></div>')  // Expected output from source
})