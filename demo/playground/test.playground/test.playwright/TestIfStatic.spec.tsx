...
test('If Static component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        // [Implementation based on source file: TestIfStatic.tsx]

        // Create the component element using h() function - static content
        const element = h('div', null,
            h('h3', null, 'If - Static'),
            // First If with when=true shows child (true case)
            h(If, { when: true } as any, h('p', null, 'true')),
            // Second If with when=false shows no child (false case)
    ...
    await expect(innerHTML).toBe('<h3>If - Static</h3><p>true</p>')  // Expected output from source
})