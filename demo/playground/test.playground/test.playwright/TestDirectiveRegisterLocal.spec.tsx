...
test('Directive Register Local component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        // [Implementation based on source file: TestDirectiveRegisterLocal.tsx]

        // Create directive
        const model: any = (element: any, arg1: any, arg2: any) => {
            const value = `${arg1} - ${arg2}`
            element.value = value
            element.setAttribute('value', value)
        }
        const Model: any = (createDirective as any)('modelLocal', model)
        Model.register()

        // Create the component element using h() function - static content
        const element = h('div', null,
            h('h3', null, 'Directive'),
            h('input', { value: 'foo', 'use:modelLocal': ['bar', 'baz'] } as any)
        )
    ...
    await expect(innerHTML).toBe('<h3>Directive</h3><input value="bar - baz">')  // Expected output from source
})