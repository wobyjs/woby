...
declare global {
    interface Window {
        testTestAttributeStatic: import('woby').Observable<undefined>
    }
}

test('Attribute - Static component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        // [Implementation based on source file: TestAttributeStatic.tsx]

        // Create the component element using h() function - static content
        const element = h('div', null,
            h('h3', null, 'Attribute - Static'),
            h('p', { 'data-color': 'red' } as any, 'content')
        )
    ...
    await expect(innerHTML).toBe('<h3>Attribute - Static</h3><p data-color="red">content</p>')  // Expected output from source
})
