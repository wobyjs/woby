...
test('Attribute Boolan - Static component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        // [Implementation based on source file: TestAttributeBooleanStatic.tsx]

        // Create the component element using h() function - static content
        const element = h('div', null,
            h('h3', null, 'Attribute Boolan - Static'),
            h('p', { disabled: true } as any, 'content'),
            h('p', { disabled: false } as any, 'content')
        )
    ...
    await expect(innerHTML).toBe('<h3>Attribute Boolan - Static</h3><p disabled="">content</p><p>content</p>')  // Expected output from source
})
