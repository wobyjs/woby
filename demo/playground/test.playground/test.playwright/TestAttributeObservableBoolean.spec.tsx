...
test('Attribute Observable Boolean component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        // [Implementation based on source file: TestAttributeObservableBoolean.tsx]

        // Create observable with initial value false
        const o = $(false)

        // Create the component element using h() function - static content
        const element = h('div', null,
            h('h3', null, 'Attribute - Observable Boolean'),
            h('p', { 'data-red': o }, 'content')  // Use observable for data-red attribute
        )
    ...
    await expect(innerHTML).toBe('<h3>Attribute - Observable Boolean</h3><p data-red="false">content</p>')  // Expected output when initial value is false
})