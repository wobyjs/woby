...
test('TestEventClickCaptureObservable component', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        const element = h('div', null,
            h('h3', null, 'Event - Click Capture Observable'),            h('p', {}, "[observable-content]")
        )
        
        // Render to body
        render(element, document.body)
        
        // Define plus2 function
        const plus2 = () => o(prev => {
            // Toggle logic would be implemented based on source
            return typeof prev === 'boolean' ? !prev : typeof prev === 'number' ? prev + 1 : prev + '_updated'
        })
        
        // Define minus1 function
        const minus1 = () => o(prev => {
            // Toggle logic would be implemented based on source
            return typeof prev === 'boolean' ? !prev : typeof prev === 'number' ? prev + 1 : prev + '_updated'
        })
        ;(document.body as any)['plus2TestEventClickCaptureObservable'] = plus2
        ;(document.body as any)['minus1TestEventClickCaptureObservable'] = minus1
    })
    ...
    // Manually trigger the plus2 function 4 times (replacing useInterval)
    await page.evaluate(() => {
        (document.body as any)['plus2TestEventClickCaptureObservable']()
    })
