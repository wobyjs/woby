...
export type ChildWithMetadata<T = unknown> = (() => Child) & {
    metadata: T;
};
...
export type ConstructorWith<T = unknown, Arguments extends unknown[] = []> = {
...
export type ContextProvider<T = unknown> = (props: {
...
export type ContextWithDefault<T = unknown> = Context<T> & {
...
export type DirectiveProvider = (props: {
    children: Child;
}) => Child;
...
export type Directive<Arguments extends unknown[] = []> = {
    Provider: DirectiveProvider;
    ref: DirectiveRef<Arguments>;
    register: DirectiveRegister;
};
export type DirectiveData<Arguments extends unknown[] = []> = {
...
export type FragmentFragment = {
    values: Fragment;
    fragmented: true;
    length: 1;
};
export type FragmentNodes = {
    values: Node[];
    fragmented?: false;
    length: 2 | 3 | 4 | 5;
};
export type FragmentFragments = {
    values: Fragment[];
    fragmented: true;
    length: 2 | 3 | 4 | 5;
};
export type FragmentMixed = {
    values: (Node | Fragment | Comment | CommentSSR)[];
    fragmented: true;
    length: 2 | 3 | 4 | 5;
};
...
export type LazyFetcher<P = {}> = () => Promise<{
    default: ComponentFunction<P>;
} | ComponentFunction<P>>;
export type LazyResult<P = {}> = LazyComponent<P> & ({
...
export type ResourceStaticPending<T = unknown> = {
    pending: true;
    error?: never;
    value?: never;
    latest?: T;
};
export type ResourceStaticRejected = {
    pending: false;
    error: Error;
    value?: never;
    latest?: never;
};
export type ResourceStaticResolved<T = unknown> = {
    pending: false;
    error?: never;
    value: T;
    latest: T;
...
export type TargetedEvent<T extends EventTarget = EventTarget, TypedEvent extends Event = Event> = Omit<TypedEvent, 'currentTarget'> & {
...
export type TargetedChangeEvent<T extends EventTarget> = TargetedEvent<T, Event & {
    target: T;
}>;
...
export type EventHandler<Event extends TargetedEvent> = {
